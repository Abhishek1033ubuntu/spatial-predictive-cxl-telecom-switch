...
Repository: spatial-predictive-cxl-telecom-switch
License: MIT
"""
...
# -------------------------------------------------------------------------
t = np.linspace(0, 180, 180)  # 3-hour commute window (180 minutes)
beta_i = 50.0
A_k = 400.0
tau_k = 90.0
sigma_k = 15.0

# Spatial population density & velocity vector
...
# Correct Predictive Lookahead Allocation (Rolling 25-minute future max envelope)
lead_steps = 25
alpha_headroom = 1.18

cxl_capacity = np.zeros_like(lambda_t)
for i in range(len(lambda_t)):
    future_window = lambda_t[i : min(i + lead_steps + 1, len(lambda_t))]
    cxl_capacity[i] = alpha_headroom * np.max(future_window)
...
plt.plot(
    ...
    color='#d62728',
    linewidth=2.5,
    label=r'Ingress Traffic Demand $\lambda_i(t)$',
)
plt.plot(
    t,
    cxl_capacity,
    ...
    linewidth=2.0,
    linestyle='--',
    label=r'CXL Predictive Envelope (25-min Lead)',
)
plt.fill_between(t, 0, lambda_t, color='#d62728', alpha=0.1)
plt.title(
    'Figure 1: Spatial-Predictive Capacity Allocation Horizon',
    fontsize=12,
    fontweight='bold',
    pad=12,
)
plt.xlabel('Simulation Time (Minutes)', fontsize=10)
plt.ylabel(r'Traffic Rate (Packets / ms)', fontsize=10)
...
# -------------------------------------------------------------------------
t_prop = 20.75  # Optical propagation delay (4,150 km fiber)
np.random.seed(42)
...
plt.plot(
    ...
    color='#e377c2',
    linewidth=2.0,
    label=r'Traditional OS Kernel Stack (Variable Jitter)',
)
plt.plot(
    t,
    cxl_latency,
    color='#17becf',
    linewidth=2.5,
    label=r'Spatial-Predictive CXL Fast-Path (Deterministic)',
)
plt.title(
    'Figure 2: One-Way Latency Bounds & Jitter Compression',
    fontsize=12,
    fontweight='bold',
    pad=12,
)
plt.xlabel('Simulation Time (Minutes)', fontsize=10)
plt.ylabel(r'One-Way Latency (ms)', fontsize=10)
plt.ylim(18, 35)
plt.legend(loc='upper right', frameon=True)
plt.tight_layout()
...
plt.plot(
    ...
    linewidth=2.5,
    marker='s',
    label=r'Spatial-Predictive CXL Switch TCO ($M)',
)
plt.fill_between(
    years,
    cxl_tco,
    ...
    color='#20c997',
    alpha=0.2,
    label=r'Net Operator Savings ($27.9M Year 5)',
)
plt.title(
    'Figure 3: 5-Year Cumulative Total Cost of Ownership (TCO)',
    fontsize=12,
    fontweight='bold',
    pad=12,
)
plt.xlabel('Deployment Timeframe (Years)', fontsize=10)
plt.ylabel(r'Cumulative Cost ($ Millions USD)', fontsize=10)
plt.xticks(years)
